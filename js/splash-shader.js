// Mounts the real @paper-design/shaders-react Warp shader into every
// [data-shader] container (the splash and the flip-menu screen share the
// same look), loaded straight from a CDN as native ES modules — no build
...
(async () => {
  const containers = document.querySelectorAll('[data-shader]');
  ...
  try {
    ...
    containers.forEach((container) => {
      createRoot(container).render(React.createElement(Warp, warpProps));
    });
  } catch (err) {
    console.warn('[milk studio] Shader background failed to load — using static background.', err);
  }
})();
